...
public class CDASampler {
	...
	protected Map<String, String> extractSampleData(String property) {
		...
		for (String name : new ArrayList<String>(samples.keySet())) {
			...
			if (name.startsWith(property, toffset) &&
					(toffset + property.length() == name.length() || name.charAt(toffset + property.length()) == '(' &&
							result.isEmpty() || name.charAt(toffset + property.length()) == '/')) {
	...
	public Property descentProperty(EStructuralFeature eStructuralFeature, final Class umlClass) {
		...
		System.out.println("Failed descending " + umlContext + " in cda:" +
				eStructuralFeature.getEContainingClass().getName() + "(" + umlClass.getName() + ")");
		return null;
	}
