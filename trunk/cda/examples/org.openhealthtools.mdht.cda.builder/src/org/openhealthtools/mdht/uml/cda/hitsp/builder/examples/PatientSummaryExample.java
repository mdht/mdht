...
public class PatientSummaryExample {
	...
	public static void main(String[] args) {
		...
		try {
			...
			PatientSummary patientSummaryDocument = patientSummarybuilder.with(familyHistorySection).with(
				functionalStatusSection).with(immunizationsSection).with(medicalEquipmentSection).with(
				medicationsSection).with(payersSection).with(planOfCareSection).with(socialHistorySection).with(
				vitalSignsSection).with(advanceDirectivesSection).with(encountersSection).buildDocument();
