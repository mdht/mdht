...
private int getRowIndex(TreeItem item) {
	int index = -1;
	if (item != null) {
		index = (item.getParentItem() == null) ? tree.indexOf(item)
...
private int getParentItemCount(TreeItem item) {
	int count = 0;
	if (item != null) {
		count = (item.getParentItem() == null) ? tree.getItemCount()
...
private TreeItem getPreviousSibling(TreeItem item) {
	...
	Widget parent = item.getParentItem() == null ? (Widget)tree : item.getParentItem();
...
void keyDown(Event event) {
	...
	switch (event.keyCode) {
		case SWT.ARROW_UP :
			...
			else if (previousRow != null && previousRow.getExpanded()
					&& previousRow.getItemCount() > 0) {
        ...
        case SWT.ARROW_RIGHT :
        	{	
	        	...
	        	TreeColumn col = newColumnIndex == -1 || tree.getColumnCount() == 0 ? null : tree.getColumn(newColumnIndex);
