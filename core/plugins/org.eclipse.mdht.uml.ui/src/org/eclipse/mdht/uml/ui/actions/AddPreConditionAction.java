...
public class AddPreConditionAction extends UML2AbstractAction {

	public AddPreConditionAction() {
		super();
	}
		...
		try {
			...
			if (Operation.class.isInstance(element)) {
				IUndoableOperation operation = new AbstractEMFOperation(
					editingDomain, UML2UIMessages.AddUMLPreConditionConstraint_operation_title) {
					@Override
					protected IStatus doExecute(IProgressMonitor monitor, IAdaptable info) {
						Constraint constraint = UMLFactory.eINSTANCE.createConstraint();

						String name = UML2UIMessages.AddUMLPreConditionConstraint_default_name;
						constraint.setName(name);
						((Operation) element).getPreconditions().add(constraint);
