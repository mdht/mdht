...
public abstract class XSDTransformUtil {
	...
	public static void initializeModel(Package umlModel) {
		...
		Stereotype schemaStereotype = XSDProfileUtil.applyXSDStereotype(umlModel, IXSDProfileConstants.SCHEMA);
		if (schemaStereotype != null) {
			umlModel.setValue(schemaStereotype, IXSDProfileConstants.SCHEMA_TARGET_NAMESPACE, "urn:hl7-org:v3");
		}

		// assign xsdGen stereotype and elementNameMapping property
		Stereotype xsdGenStereotype = XSDProfileUtil.applyXSDGenerationStereotype(
			umlModel, IXSDGenerationConstants.SCHEMA_GEN);
		if (xsdGenStereotype != null) {
			umlModel.setValue(
				xsdGenStereotype, IXSDGenerationConstants.SCHEMA_GEN_ELEMENT_NAME_MAPPING,
	...
	public static Profile getXSDProfile(EObject eObject) {
		...
		if (eResource != null) {
			...
			if (resourceSet != null) {
				return (Profile) UMLUtil.load(
					resourceSet, URI.createURI(XMLmodelingResource.XSD_PROFILE_URI), UMLPackage.Literals.PROFILE);
			}
		}
	...
	public static String toLowerCamelCase(String name) {
		...
		while (idx < name.length() && Character.isUpperCase(name.charAt(idx))) {
			if (idx > 0 && (idx + 1 < name.length()) && Character.isLowerCase(name.charAt(idx + 1))) {
