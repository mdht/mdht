...
if [ "x" == "x$JAVA_HOME" ]; then JAVA_HOME=/usr/bin/java; fi
...
if [ "x" == "x$MDHT_PDELOCATION" ]; then MDHT_PDELOCATION=/home/seanmuir/releng/org.eclipse.releng.basebuilder; fi
...
# if [ "x" == "x$MDHT_ECLIPSE" ]; then MDHT_ECLIPSE=/home/seanmuir/galileo64clean/eclipse; fi

# Root Directory for both the Build (src files) and the eventual Repository
if [ "x" == "x$MDHT_LOCATION" ]; then MDHT_LOCATION=$PWD; fi

# src location
if [ "x" == "x$MDHT_BUILDDIRECTORY" ]; then MDHT_BUILDDIRECTORY=$MDHT_LOCATION/helpsrc; fi

# Repository/Update Site location
if [ "x" == "x$MDHT_REPODIRECTORY" ]; then MDHT_REPODIRECTORY=$MDHT_LOCATION/helprepo; fi
...
# Build Target - N-Nightly; I-Integration; etc
if [ "x" == "x$MDHT_COMPONENT" ]; then MDHT_COMPONENT=CDAMODELSDOC; fi
