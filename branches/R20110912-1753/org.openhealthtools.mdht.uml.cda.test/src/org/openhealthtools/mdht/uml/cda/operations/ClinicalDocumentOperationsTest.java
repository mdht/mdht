	...
	private static final CDATestCase TEST_CASE_ARRAY[] = {
			...
			new CDAAddTestCase("Section") {
				...
				protected void doTest(final EObject objectToTest, final BasicDiagnostic diagnostician,
					...
					ClinicalDocumentOperations.addSection((ClinicalDocument) objectToTest, section);

					assertTrue(ClinicalDocumentOperations.getSections((ClinicalDocument) objectToTest).get(0).equals(
						section));
				}
			...
			new CDAGetTestCase("Section") {
				...
				protected Object doGet(final EObject objectToTest) {
					return ClinicalDocumentOperations.getSections((ClinicalDocument) objectToTest);
				}
			...
			new CDAHasTestCase("Code") {
				...
				protected void doTest(final EObject objectToTest, final BasicDiagnostic diagnostician,
					...
					final CE code = DatatypesFactory.eINSTANCE.createCE(CODE, CODE_SYSTEM);
				...
				protected boolean doHas(final EObject objectToTest, final String templateId) {
					return ClinicalDocumentOperations.hasCode(
						(ClinicalDocument) objectToTest, CODE, CODE_SYSTEM, CODE_SYSTEM_NAME);
				}
			...
			new CDAHasTestCase("TemplateID") {
				...
				protected void doTest(final EObject objectToTest, final BasicDiagnostic diagnostician,
						final Map<Object, Object> map) {
					((ClinicalDocument) objectToTest).getTemplateIds().add(THE_II);
					final boolean hasIsGood = doHas(objectToTest, TEMPLATE_ID);
					assertTrue("Has \"" + getTestTargetDescription() + "\" failed for \"" +
							objectToTest.eClass().getName() + "\"", hasIsGood);
				}
				...
				@Override
				protected boolean doHas(final EObject objectToTest, final String templateId) {
					return ClinicalDocumentOperations.hasTemplateId((ClinicalDocument) objectToTest, templateId);
				}
			...
			new CDATestCase("SetStructureBody") {
				...
				protected void doTest(final EObject objectToTest, final BasicDiagnostic diagnostician,
					...
					ClinicalDocumentOperations.setStructuredBody((ClinicalDocument) objectToTest, sb);
					assertTrue(((ClinicalDocument) objectToTest).getComponent().getStructuredBody().equals(sb));
				}
			},
			...
			new CDATestCase("GetPatients") {
				...
				protected void doTest(final EObject objectToTest, final BasicDiagnostic diagnostician,
						final Map<Object, Object> map) {
					assertTrue(ClinicalDocumentOperations.getPatients((ClinicalDocument) objectToTest).isEmpty());
					final Patient patient = CDAFactory.eINSTANCE.createPatient();
					ClinicalDocumentOperations.addPatient((ClinicalDocument) objectToTest, patient);
					assertTrue(!ClinicalDocumentOperations.getPatients((ClinicalDocument) objectToTest).isEmpty());
				}
			},
			...
			new CDATestCase("GetPatientRoles") {
				...
				protected void doTest(final EObject objectToTest, final BasicDiagnostic diagnostician,
						final Map<Object, Object> map) {
					assertTrue(ClinicalDocumentOperations.getPatientRoles((ClinicalDocument) objectToTest).isEmpty());
					final PatientRole patientRole = CDAFactory.eINSTANCE.createPatientRole();
					ClinicalDocumentOperations.addPatientRole((ClinicalDocument) objectToTest, patientRole);
					assertTrue(!ClinicalDocumentOperations.getPatientRoles((ClinicalDocument) objectToTest).isEmpty());
				}
			},
			// HasSectionTemplate
