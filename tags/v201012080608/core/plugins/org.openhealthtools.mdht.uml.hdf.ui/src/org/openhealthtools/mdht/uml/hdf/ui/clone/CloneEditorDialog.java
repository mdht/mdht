...
public class CloneEditorDialog extends SelectionStatusDialog {
	...
	private class OverrideFlatTreeAction extends Action {
		...
		public OverrideFlatTreeAction() {
			...
			URL disableURL = 
				Activator.getDefault().getBundle().getEntry(ICONS_PATH+"//dlcl16//impl_co.gif");
			ImageDescriptor disableImage = ImageDescriptor.createFromURL(disableURL);
			setDisabledImageDescriptor(disableImage);
			
			URL enableURL = 
				Activator.getDefault().getBundle().getEntry(ICONS_PATH+"//elcl16//impl_co.gif");
     ...
     */
    public CloneEditorDialog(Shell parent,
    		CloneEditorViewLabelProvider labelProvider, 
    		CloneEditorViewContentProvider contentProvider,
            EObject input) {
        super(parent);
...
//    }
	 ...
	 *             {@link CloneEditorDialog#setComparator(ViewerComparator)}
	 *             instead
	 */
...
//		PlatformUI.getWorkbench().getHelpSystem().setHelp(newShell, IJavaHelpContextIds.OVERRIDE_TREE_SELECTION_DIALOG);
	...
	public Class getSelectedClass(){
		return ((CloneEditorTreeViewer)fContentProvider.getViewer()).getSelectedClass();
	}
