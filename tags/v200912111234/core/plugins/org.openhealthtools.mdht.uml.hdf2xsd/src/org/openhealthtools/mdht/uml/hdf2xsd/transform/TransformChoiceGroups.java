		...
		for (Iterator iter = specializations.iterator(); iter.hasNext();) {
			...
			for (String traversalName : traversalNames) {
				...
				Association choiceAssociation = choiceGroup.createAssociation(
						true, AggregationKind.NONE_LITERAL, traversalName, 1, 1, choiceMember,
						false, AggregationKind.NONE_LITERAL, null, 1, 1);
				
				// make the choice element nillable
				Property traversalProperty = (Property) choiceAssociation.getMember(traversalName);
				Stereotype xsdStereotype = XSDProfileUtil.applyXSDStereotype(traversalProperty, IXSDProfileConstants.ELEMENT);
			...
			for (Property targetProperty : choiceClassAttributes) {
				Property existingProperty = choiceMember.getOwnedAttribute(targetProperty.getName(), targetProperty.getType());
				if (existingProperty != null) {
					// already copied down from another choice group use
					continue;
				}
				Property targetCopy = (Property) EcoreUtil.copy(targetProperty);
				...
				if (targetProperty.getAssociation() != null) {
					Association memberAssoc = (Association) EcoreUtil.copy(targetProperty.getAssociation());
					...
					for (Property originProperty : targetProperty.getAssociation().getOwnedEnds()) {
						Property originCopy = (Property) EcoreUtil.copy(originProperty);
