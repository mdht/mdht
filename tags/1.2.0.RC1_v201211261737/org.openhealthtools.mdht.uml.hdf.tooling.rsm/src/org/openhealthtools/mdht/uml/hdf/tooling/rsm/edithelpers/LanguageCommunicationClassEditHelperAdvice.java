 ...
 * @generated
 */
public class LanguageCommunicationClassEditHelperAdvice extends RIMBaseEditHelperAdvice {
	...
	protected ICommand getBeforeConfigureCommand(final ConfigureRequest request) {
		final String stereotypeName = (String) request.getParameter(EditRequestParameters.APPLIED_STEREOTYPE_PARAM_NAME);
		if (stereotypeName != null) {
			final EObject eObject = request.getElementToConfigure();
			if (eObject instanceof Element && ((Element) eObject).getAppliedStereotype(stereotypeName) != null) {
				request.setParameter(EditRequestParameters.APPLIED_STEREOTYPE_PARAM_NAME, null);
			}
		}
	...
	protected String getLocalizedName(EObject eObject) {
		...
		Stereotype stereotype = element.getAppliedStereotype(RIMElementTypes._LANGUAGECOMMUNICATION__CLASS.getStereotypeName());
	 ...
	 */
	@Override
	protected ICommand getBeforeReorientReferenceRelationshipCommand(final ReorientReferenceRelationshipRequest request) {
		...
		if (request.getNewRelationshipEnd() != null && request.getOldRelationshipEnd() != null &&
				request.getReferenceOwner() != null && sourceFeatureName != null && elementType != null) {
			if (ReorientRequest.REORIENT_TARGET == request.getDirection()) {
				if (!RIMElementTypes.matchesTarget(elementType, request.getNewRelationshipEnd()) ||
						relationshipAlreadyExists(request, sourceFeatureName, elementType)) {
					return UnexecutableCommand.INSTANCE;
				} else {
					return getReorientCommand(request, sourceFeatureName, elementType);
				}
			} else {
				if (!RIMElementTypes.matchesSource(elementType, request.getNewRelationshipEnd()) ||
						relationshipAlreadyExists(request, sourceFeatureName, elementType)) {
					return UnexecutableCommand.INSTANCE;
				} else {
					return getReorientCommand(request, sourceFeatureName, elementType);
	...
	protected ICommand getBeforeDestroyReferenceCommand(final DestroyReferenceRequest request) {
		...
		if (elementType != null && sourceFeatureName != null) {
			result.add(new AbstractTransactionalCommand(
				request.getEditingDomain(), request.getLabel(), Collections.EMPTY_LIST) {
