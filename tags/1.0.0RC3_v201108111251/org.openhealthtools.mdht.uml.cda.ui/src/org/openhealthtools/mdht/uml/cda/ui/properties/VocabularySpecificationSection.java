 ...
 * $Id: $
 */
public class VocabularySpecificationSection extends ResettableModelerPropertySection {
	...
	private void modifyFields() {
		if (!(idModified || nameModified || versionDateModified || codeModified || displayNameModified)) {
			return;
		}
				...
				protected IStatus doExecute(IProgressMonitor monitor, IAdaptable info) {
					...
					if (stereotype == null) {
						...
						if (stereotype != null) {
							...
							property.setValue(
								stereotype, ICDAProfileConstants.VOCAB_SPECIFICATION_CODE_SYSTEM, value.length() > 0
										? value
										: null);
						}
						...
						if (stereotype != null) {
							...
							property.setValue(
								stereotype, ICDAProfileConstants.VOCAB_SPECIFICATION_CODE_SYSTEM_NAME,
								value.length() > 0
										? value
										: null);
						}
						...
						if (stereotype != null) {
							...
							property.setValue(
								stereotype, ICDAProfileConstants.VOCAB_SPECIFICATION_CODE_SYSTEM_VERSION,
								value.length() > 0
										? value
										: null);
						}
						...
						if (stereotype != null) {
							...
							property.setValue(
								stereotype, ICDAProfileConstants.VOCAB_SPECIFICATION_CODE, value.length() > 0
										? value
										: null);
						}
						...
						if (stereotype != null) {
							...
							property.setValue(
								stereotype, ICDAProfileConstants.VOCAB_SPECIFICATION_DISPLAY_NAME, value.length() > 0
										? value
										: null);
						}
					} else {
	...
	protected void resetFields() {
		...
		try {
			...
			IUndoableOperation operation = new AbstractEMFOperation(editingDomain, "Restore Default Values") {
				@Override
				protected IStatus doExecute(IProgressMonitor monitor, IAdaptable info) {
					Stereotype vocabSpecificationStereotype = CDAProfileUtil.applyCDAStereotype(
						property, ICDAProfileConstants.VOCAB_SPECIFICATION);

					if (vocabSpecificationStereotype == null) {
						return Status.CANCEL_STATUS;
					}

					property.unapplyStereotype(vocabSpecificationStereotype);
	...
	public void createControls(final Composite parent, final TabbedPropertySheetPage aTabbedPropertySheetPage) {
		...
		Composite composite = getWidgetFactory().createGroup(parent, "Vocabulary Specification");
		...
		CLabel idLabel = getWidgetFactory().createCLabel(composite, "Code System ID:"); //$NON-NLS-1$
		data = new FormData();
		...
		CLabel nameLabel = getWidgetFactory().createCLabel(composite, "Code System Name:"); //$NON-NLS-1$
		data = new FormData();
	...
	public void refresh() {
		...
		if (stereotype != null) {
			...
			displayNameText.setText(name != null
					? name
					: "");
		} else {
